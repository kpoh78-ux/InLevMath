// apps/web/scripts/test-kiosk-attendance.mjs
...
async function runVerification() {
  ...
  try {
    // 1. 테스트용 재원 학생 및 학부모 데이터 조회 또는 준비
    ...
    if (!testStudent) {
      console.log('⚠️ 테스트용 재원 학생이 없어 임시 학생을 생성합니다.');
      ...
      if (!teacher) {
        throw new Error('교사 계정이 데이터베이스에 존재해야 합니다.');
      }
    ...
    const testPin = testStudent.attendancePin || testStudent.user.phone.slice(-4);
    ...
    // ─────────────────────────────────────────────────────────────
    console.log('👉 [시나리오 1] 학생이 키오스크에 4자리 PIN [3630] 1차 입력 (등원)');
    
    // API 라우트 로직 시뮬레이션
    const { handleKioskPin, confirmCheckOut } = await import('../src/lib/attendanceService.js');
    ...
    // ─────────────────────────────────────────────────────────────
    console.log('👉 [시나리오 2] 등원 상태에서 학생이 키오스크에 4자리 PIN [3630] 2차 입력 (수업 종료 후)');
    ...
    // ─────────────────────────────────────────────────────────────
    console.log('👉 [시나리오 4] 하원 완료 후 다시 PIN [3630] 입력 시');
