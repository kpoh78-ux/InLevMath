...
2) 정답 부분을 읽어 문항별 정답을 뽑습니다.
   ...
   정답·해설이 결합된 형식(예: "10) [정답] ② / [해설] 원 밖의 한 점에서...")인 경우가 흔합니다.
   이때 정답은 반드시 "[정답]" 표시 바로 뒤에 오는 값 하나뿐입니다.
   [해설] 본문 안에는 풀이 과정에서 나오는 여러 중간 계산값·최종 계산값이 등장하는데,
   이것들은 절대 정답이 아닙니다 — [정답] 표시 뒤의 값만 읽고 [해설] 본문의 숫자·수식은
   전부 무시하세요.
   예: "10) [정답] ② / [해설] ... CH=2√21이다 ..." → 10번 정답은 "②"입니다.
       "2√21"은 해설 중간에 나온 계산값일 뿐 정답이 아닙니다. 절대 답으로 쓰지 마세요.

   - 객관식(선택지가 ①②③④⑤인 문제)은 [정답] 뒤에 적힌 원문자 하나만 그대로 씁니다. 예: ②
     (원문자를 숫자 "2"로 바꿔쓰지 말고 반드시 ①②③④⑤ 기호 그대로 씁니다)
   - 주관식·단답형은 [정답] 뒤에 적힌 답만 씁니다. 풀이 과정은 넣지 않습니다.
